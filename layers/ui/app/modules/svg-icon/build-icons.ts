...
interface SvgIconsModuleOptions {
	iconsDir: string
	runtimeIconNamesPath: string
	rootGroupTitle: string
	recursive: boolean
}

const normalizeIconName = (path: string) => path
	.replace(/\.svg$/i, '')
	.replace(/[\\/]/g, '-')
	.replace(/[^a-zA-Z0-9-]/g, '-')
	.replace(/-+/g, '-')
...
const getSvgIconFiles = (dir: string, baseDir = dir, recursive = true): string[] => readdirSync(dir)
	.flatMap((name) => {
...
const getSvgIconData = (content: string): SvgIconData => {
...
const getIconFiles = (iconsDir: string, recursive: boolean): IconFile[] => existsSync(iconsDir)
...
const buildIconNamesContent = (files: IconFile[], rootGroupTitle: string) => {
...
const writeIconNames = (iconsDir: string, iconNamesPath: string, rootGroupTitle: string, recursive: boolean) => {
...
const resolvePath = (path: string, resolve: (path: string) => string) => isAbsolute(path)
	? path
	: resolve(path)

export default defineNuxtModule<SvgIconsModuleOptions>({
	meta: {
		name: 'svg-icons',
		configKey: 'svgIcons',
	},
	...
	setup(options, nuxt) {
		...
		if (nuxt.options.dev) {
			let writeIconNamesTimeout: ReturnType<typeof setTimeout> | undefined
